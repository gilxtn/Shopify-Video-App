...
export const action = async ({ request }) => {
  ...
  const result = await response.json();
  // console.log(result?.choices?.[0],"result p=====")
...
export default function ProductTable() {
  ...
  //   modalProduct?.extendedInfo?.find((v) => v.isMain)?.videoUrl || "",
  // );
  const [previewVideo, setPreviewVideo] = useState("");
  ...
  const [failedVideoProducts, setFailedVideoProducts] = useState([]);
  const [isManualOnlyMode, setIsManualOnlyMode] = useState(false);
  const shouldShowVideoSection = !isManualOnlyMode || !!previewVideo;
  ...
  useEffect(() => {
    if (summaryFetcher.data && summaryFetcher.state === "idle") {
      console.log(summaryFetcher.data, "summaryFetcher.data______");
      try {
        ...
        shopify.toast.show(
          "Unable to generate video summary. Please try again.",
          { isError: true },
        );
        setIsGeneratingSummary(false);
      }
  ...
  const activeVideoSummary = useMemo(() => {
    return (
      modalProduct?.extendedInfo?.find((info) => info.videoUrl === previewVideo)
        ?.aiSummary || ""
    );
  ...
  const getUrlByMode = (product, mode) => {
    if (!product) return "";

    const extended = product.extendedInfo || [];
    const metafieldUrl = product.metafield?.value || "";
    ...
    if (mode === "auto") {
      ...
      const anyAuto =
        autoNonMain ||
        extended.find(
          (info) => info && info.source_method === "AUTO" && info.videoUrl,
        ) ||
        null;

      return anyAuto?.videoUrl || metafieldUrl;
    }
    ...
    if (mode === "manual") {
      ...
      const anyManual =
        manualMain ||
        extended.find(
          (info) => info && info.source_method === "MANUAL" && info.videoUrl,
        ) ||
        null;

      if (anyManual?.videoUrl) {
        return anyManual.videoUrl;
      }
      const autoNonMain =
        extended.find(
          (info) =>
  ...
  const sortOptions = [
    ...
    {
      label: "Last Updated",
      value: "updatedAtVideo asc",
      directionLabel: "Oldest first",
    },
    {
      label: "Last Updated",
      value: "updatedAtVideo desc",
      directionLabel: "Newest first",
    },
  ];
  ...
  const handleVideo = async (ids) => {
    try {
      ...
      for (const id of idsOnly) {
        const getGidForNumericId = (numericId) => {
          return (
            products.find((p) => p.id.endsWith(`/${numericId}`))?.id ||
        ...
        try {
          ...
          if (
            Array.isArray(data.erroredProducts) &&
            data.erroredProducts.length
          ) {
            const titles = data.erroredProducts
              .map((p) => p.title || p.id)
              .join(", ");
            shopify.toast.show(`Couldn't find videos for ${titles}`, {
              isError: true,
            });
          }
          ...
          if (updated) {
            ...
            shopify.toast.show(
              `Video generated for ${updated.productTitle || "product"}`,
              { isError: false },
            );
            ...
            const gid = getGidForNumericId(id);
  ...
  const validateYouTubeVideo = async (videoLink) => {
    ...
    try {
      ...
      if (!validUrl.ok) {
        return {
          success: false,
          message: "This YouTube video does not exist or is private.",
        };
      }
      ...
      return {
        success: false,
        message: "Could not verify video. Please try again.",
      };
    }
  };
  ...
  const markVideoOpened = async (product) => {
    try {
      ...
      console.error("Failed to mark video opened:", e);
      shopify.toast.show("Could not mark as viewed", { isError: true });
    }
  };
  const hasManualSaved = useMemo(() => {
    if (!modalProduct?.extendedInfo) return false;

    return modalProduct.extendedInfo.some(
      (info) =>
        info &&
        info.isMain === true &&
        info.source_method === "MANUAL" &&
  ...
  const isSwitchDisabled = useMemo(() => {
    if (!modalProduct) return true;
    if (isManualOnlyMode) {
      return radioValue[0] === "manual";
    }
    if (radioValue[0] === "auto") {
      return false;
    }
    return !canSwitchToAuto;
  }, [modalProduct, isManualOnlyMode, radioValue, canSwitchToAuto]);
  return (
    <>
      {!findCharge && pageShow ? (
        ...
        pageShow && (
          <Page title="Product Table with YouTube URLs" fullWidth>
                      ...
                      return (
                        ...
                        >
                            ...
                            <InlineStack align="" gap="200" blockAlign="center">
                              {loadingProduct[numericProductId] ? (
                                ...
                                <>
                                  ...
                                  <Button
                                    id={`preview-edit-button-${product.id.split("/").pop()}`}
                                    variant={"primary"}
                                    // icon={hasSeenVideo && LogoYoutubeIcon}
                                    icon={LogoYoutubeIcon}
                                    // tone={hasSeenVideo ? "critical" : "success"}
                                    onClick={() => {
                                      setIsManualOnlyMode(false); // normal full mode
                              ...
                              {videoSourceValue && (
                                ...
                                <div
                                  ...
                                  className="video-tooltip-wrapper"
                                >
                                  <style>
                          ...
                          <IndexTable.Cell>
                            {inventoryStatus
                              ?.toLowerCase()
                              .startsWith("0 in stock") ||
                            inventoryStatus
                              ?.toLowerCase()
                              .startsWith("out of stock") ? (
                              <Text variant="bodySm" as="span">
                                <span
                                  style={{ color: "#881919", fontWeight: 500 }}
                                >
                                  {inventoryStatus.split(" ")[0]}{" "}
                                  {inventoryStatus.split(" ")[1]}{" "}
                                  {inventoryStatus.split(" ")[2]}
                                </span>{" "}
                              ...
                              <style>
                                {`
                                  ...
                                  .automatic-block:after{  content: "";  width:100%;  left:0;  top:0;  height:100%;
                                    position: absolute;  z-index: 1;  right: 0;  bottom: 0;  pointer-events: none;  background: rgba(0,0,0,0.5);
                                  }
            ...
            <Modal id="demo-modal">
                  ...
                  {!isManualOnlyMode && (
                    <InlineStack align="end">
                      <Button
                        ...
                        onClick={() => {
                          if (isSwitchDisabled) return; // safety

                          const nextMode =
                            radioValue[0] === "auto" ? "manual" : "auto";

                          setRadioValue([nextMode]);

                          if (modalProduct) {
                            let url = getUrlByMode(modalProduct, nextMode);
                            if (!url && previewVideo) {
                              url = previewVideo;
                            }

                            setPreviewVideo(url || "");
                            seteditVideoLink(url || "");
                            setIsApplied(false);
                            setEditError("");
                          }
                        }}
                      ...
                      <Box minWidth="80%">
                        <TextField
                          ...
                          onChange={(value) => {
                            ...
                            setIsApplied(false);
                            // document
                            //   .querySelector(".automatic-block")
                            //   ?.classList.remove("automatic-block");
                          ...
                          helpText={
                            editError
                              ? ""
                              : "Format Example: https://youtube.com/embed/videoId"
                          }
                        />
                      </Box>
                      <Button
                        ...
                        onClick={async () => {
                          const result =
                            await validateYouTubeVideo(editVideoLink);
                          if (!result.success) {
                          ...
                          <div className="summary-tooltip-container">
                            <div
                              style={{
                                ...
                                transform: "rotate(314deg) scaleY(0.5)",
                                zIndex: -1,
                              }}
                            />
                            <div
              ...
              <TitleBar
                title={
                  (modalProduct?.title || modalProduct?.productTitle || "-") +
                  " , " +
                  (modalProduct?.vendor || "")
                }
              ></TitleBar>
            ...
            <Modal id="bulk-delete-modal">
              ...
              <Page>
                <BlockStack gap="200">
                  <Text variant="bodyLg" as="p">
                    Are you sure you want to delete videos for{" "}
                    {selectedResources.length} products?
                  </Text>
                </BlockStack>
                <br />
                <InlineStack wrap={false} align="end">
                  <ButtonGroup>
                    ...
                    <Button
                      onClick={() => shopify.modal.hide("bulk-delete-modal")}
                    >
                      Close
                    </Button>
                  </ButtonGroup>
